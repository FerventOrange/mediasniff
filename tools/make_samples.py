...
def packed_audio() -> None:
    ...
    body = (b"com.apple.streaming.transportStreamTimestamp\x00"
...
def aes128() -> None:
    ...
    subprocess.run(
        ["openssl", "enc", "-aes-128-cbc", "-K", key, "-iv", "00" * 16,
         "-in", src, "-out", os.path.join(HERE, "enc_aes128.ts")],
        check=True,
    )
    print("  enc_aes128.ts")
...
def webm() -> None:
    ...
    header = el("1A45DFA3",
                ...
                + el("42F3", u(8)) + el("4282", b"webm")
                + el("4287", u(2)) + el("4285", u(2)))
    info = el("1549A966", el("2AD7B1", u(1000000)) + el("4D80", b"mediasniff"))

    def trk(num: int, ttype: int, codec: str) -> bytes:
        return el("AE", el("D7", u(num)) + el("73C5", u(num))
                  + el("83", u(ttype)) + el("86", codec.encode()))
...
def llhls_and_elementary():
    ...
    for src, out, limit in (("apple_vid_seg.mp4", "llhls_part_video.m4s", 120000),
...
def flv():
    ...
    def build(has_v, has_a, tags):
        ...
        d = bytearray(b"FLV\x01" + bytes([flags]) + struct.pack(">I", 9)
                      + struct.pack(">I", 0))
